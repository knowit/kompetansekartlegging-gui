...
export const AnswerHistory = ({...props}: AnswerHistoryProps) => {
  ...
  const HistoryTreeView = ({...props}: HistoryTreeViewProps) => {
    ...
    const renderAnswer = (answer: UserAnswer) => {
      let key = String(g.next().value);
      return (
        <TreeItem key={key} nodeId={key} label={findQuestion(answer.question.id)}>
          <TreeItem nodeId={String(g.next().value)} label={"Kunnskap: " + parseScore(answer.knowledge)}/>
          <TreeItem nodeId={String(g.next().value)} label={"Motivasjon: " + parseScore(answer.motivation)}/>
        </TreeItem>
      )
    };

    let sortedData = props.data.sort( (a, b) => Date.parse(b.createdAt) - (Date.parse(a.createdAt)));
  ...
  const findQuestion = (questionId: string): string => {
    ...
    return (question ? question?.category.text + ": " + question?.topic : "Not defined");
  }
